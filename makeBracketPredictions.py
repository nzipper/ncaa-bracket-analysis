...
def extractFeatures(team_data, matchups):
    ...
    for i,row in matchups.iterrows():
        ...
        team1_stats = team_data.loc[(team_data.Season == int(year)) & (team_data.TeamID == int(ID1))].to_numpy()
        team2_stats = team_data.loc[(team_data.Season == int(year)) & (team_data.TeamID == int(ID2))].to_numpy()
